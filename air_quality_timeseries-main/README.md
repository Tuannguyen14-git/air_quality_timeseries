...
### Notes for Coursework / Grading

Project triển khai đủ 3 hướng tiếp cận:

Classification (có kiểm soát leakage)

Regression (time-aware supervised learning)

Time Series Forecasting (ARIMA – đúng lý thuyết môn học)

Tách rõ:

Notebook (giảng dạy, minh hoạ)

Module hoá (src/) để tái sử dụng

Pipeline có thể chạy tự động bằng Papermill

Không sử dụng deep learning, tập trung đúng các phương pháp trong học phần

### Limitations

ARIMA chỉ áp dụng cho chuỗi đơn biến (1 trạm)

Chưa xét yếu tố không gian (spatial correlation giữa các trạm)

Horizon dự báo dài có thể giảm độ chính xác

### Future Work

Thử SARIMA / SARIMAX (seasonality + exogenous variables)
